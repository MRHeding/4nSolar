...
// ================== QUOTATION IMPORT FUNCTIONS ==================



// Get quotation by quote number or ID

function getQuotationForPOS($quote_identifier) {
    ...
    try {
        ...
        // Try to find by quote number first, then by ID
        ...
        $stmt = $pdo->prepare("SELECT q.*, u.full_name as created_by_name 
                              ...
                              WHERE q.quote_number = ? OR q.id = ?");

        $stmt->execute([$quote_identifier, $quote_identifier]);
        ...
        // Get quote items with current inventory data

        $stmt = $pdo->prepare("SELECT qi.*, i.brand, i.model, i.size_specification, 

                              i.base_price, i.selling_price, i.stock_quantity, i.generate_serials, i.is_active,
        ...
        if (empty($quote['items'])) {
            ...
            return ['success' => false, 'message' => 'No valid items found in this quotation'];
...
// Import quotation items to POS sale

function importQuotationToPOS($sale_id, $quote_id) {
    ...
    try {
        ...
        $quote_result = getQuotationForPOS($quote_id);
        ...
        // Debug: Log the number of items found in quotation

        error_log("QUOTATION IMPORT DEBUG - Found " . count($quote['items']) . " items in quotation {$quote['quote_number']}");
        ...
        foreach ($quote['items'] as $quote_item) {
            ...
            error_log("QUOTATION IMPORT DEBUG - Processing item: {$quote_item['brand']} {$quote_item['model']} (ID: {$quote_item['inventory_item_id']}, Active: " . ($quote_item['is_active'] ? 'Yes' : 'No') . ", Stock: {$quote_item['stock_quantity']})");

            

            // Check if item exists in inventory (handle deleted items)

            if (!$quote_item['brand'] || !$quote_item['model']) {

                $errors[] = "Item with ID {$quote_item['inventory_item_id']} no longer exists in inventory";
            ...
            // Check if item is inactive and warn user

            if (!$quote_item['is_active']) {

                $errors[] = "Item '{$quote_item['brand']} {$quote_item['model']}' is inactive but will be imported";

            }

            

            // Check stock availability and warn if insufficient

            if ($quote_item['stock_quantity'] <= 0) {

                $errors[] = "Item '{$quote_item['brand']} {$quote_item['model']}' is out of stock but will be imported";

            }

            

            // Use the price that was originally selected in the quotation
            ...
            $stmt->execute([$sale_id, $quote_item['inventory_item_id']]);
            ...
            if ($existing_item) {
                ...
                $new_quantity = $existing_item['quantity'] + $quote_item['quantity'];

                $new_discount_amount = ($unit_price * $quote_item['discount_percentage'] / 100) * $new_quantity;
                ...
                $result = $stmt->execute([
                    ...
                    $new_quantity, $unit_price, $quote_item['discount_percentage'], 

                    $new_discount_amount, $new_total_amount, $existing_item['id']
                ...
                $discount_amount = ($unit_price * $quote_item['discount_percentage'] / 100) * $quote_item['quantity'];

                $total_amount = ($unit_price * $quote_item['quantity']) - $discount_amount;
                ...
                $result = $stmt->execute([
                    ...
                    $sale_id, $quote_item['inventory_item_id'], $quote_item['quantity'], 

                    $unit_price, $quote_item['discount_percentage'], $discount_amount, 

                    $total_amount, $quote_item['serial_numbers'], $quote_item['serial_count']
            ...
            if ($result) {
                ...
                $errors[] = "Failed to import item '{$quote_item['brand']} {$quote_item['model']}'";
        ...
        // Update customer information from quotation

        if (!empty($quote['customer_name']) || !empty($quote['customer_phone'])) {
